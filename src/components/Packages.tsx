...
const Packages = () => {
  ...
  return (
    <section
      id="packages"
      ref={ref}
      className={`py-16 bg-gray-800 ${inView ? 'opacity-100' : 'opacity-0'} transition-opacity duration-1000`}
    >
      <div className="container mx-auto px-4">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
          {packages.map((pkg) => (
            <div
              key={pkg.name}
              className="bg-gradient-to-br from-gray-100 to-gray-300 dark:from-gray-700 dark:to-gray-800 rounded-lg p-6 shadow-neumorph hover:shadow-xl transition-shadow flex flex-col"
            >
              <h3 className="text-xl font-semibold mb-4 text-gray-800 dark:text-white text-center">{pkg.name}</h3>
              <ul className="space-y-3 mb-6 flex-grow">
                {pkg.features.map((feature) => (
                  <li key={feature} className="flex items-center text-gray-800 dark:text-gray-300">
                    <FcDownRight className="h-5 w-5 text-blue-400 mr-2" />
                    <span>{feature}</span>
                  </li>
                ))}
              </ul>
